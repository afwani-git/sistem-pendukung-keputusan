...
func main() {
	...
	gap := gap.Gap{}
	gap.AspectTable = aspectTable
	gap.CadidateTable = candidateTable
	gap.Wtable = wTable

	result.Gap = gap
	result.GapMapping = gapMapping.MappingGap(gap)
	result.GapCalculate = calculate.CalculateFactor(result.GapMapping, gap)
	result.GapRanking = ranking.Ranking(result.GapCalculate, gap)

	// echo
	e := echo.New()
	e.GET("/init", func(c echo.Context) error {
		return c.JSON(http.StatusOK, result.Gap)
	})
